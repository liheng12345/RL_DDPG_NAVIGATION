...
class ImitationNet(nn.Module):
    ...
    def __init__(self, control_dim=2, device='cpu'):
        ...
        self.device = device
        # 定义一个神经网络从odom+scan到控制输入
        self.pose_to_control = neural_net.PoseToControl(
...
def train(model, mode='dagger'):
    ...
    for epoch in range(args.epochs):
        ...
        for pose, scan, vel in train_loader:
            ...
            opt.zero_grad()
            loss = model.step(odom_input=pose,
                              laser_scan=scan,
                              target_control=vel)
            loss.backward()
            opt.step()
...
def test(model, odom_input, laser_scan):
    odom_input = torch.tensor(odom_input).detach().to(args.device).type(
        torch.float32)
    laser_scan[laser_scan == np.inf] = 0
    laser_scan = laser_scan[:, ::30]
    laser_scan = torch.tensor(laser_scan).detach().to(args.device).type(
        torch.float32)
