...
model_dir = os.path.join(
    os.path.split(os.path.realpath(__file__))[0], 'model', 'critic')
...
class CriticNetwork:
    ...
    def create_training_method(self):
        ...
        self.cost = tf.reduce_mean(
            tf.square(self.y_input - self.q_value_output))
    ...
    def create_q_network(self, state_dim, action_dim):
        ...
        W2 = self.variable([layer1_size, layer2_size],
                           layer1_size + action_dim)
        W2_action = self.variable([action_dim, layer2_size],
                                  layer1_size + action_dim)
    ...
    def create_target_q_network(self, state_dim, action_dim, net):
        ...
        layer1 = tf.nn.relu(
            tf.matmul(state_input, target_net[0]) + target_net[1])
        layer2 = tf.nn.relu(
