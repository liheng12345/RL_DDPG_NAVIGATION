...
def _check_image_input(observation_space: spaces.Box, key: str = "") -> None:
    ...
    if observation_space.shape[
            non_channel_idx] < 36 or observation_space.shape[1] < 36:
        warnings.warn(
...
def _check_unsupported_spaces(env: gym.Env, observation_space: spaces.Space,
...
def _check_render(env: gym.Env,
                  warn: bool = True,
                  headless: bool = False) -> None:  # pragma: no cover
    """
...
def check_env(env: gym.Env,
    ...
    if warn:
        ...
        obs_spaces = observation_space.spaces if isinstance(
            observation_space, spaces.Dict) else {
                "": observation_space
            }
    ...
    if _is_numpy_array_space(observation_space) and _is_numpy_array_space(
            action_space):
        _check_nan(env)
