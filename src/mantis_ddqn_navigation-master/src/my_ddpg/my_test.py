...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, max_action):
        ...
        self.b1 = BatchNorm(training_bool, 300, 2)
        self.l2 = nn.Linear(300, 200)
        self.b2 = BatchNorm(training_bool, 200, 2)
        self.l3 = nn.Linear(200, action_dim)
        self.b3 = BatchNorm(training_bool, action_dim, 2)
    ...
    def forward(self, x):
        ...
        # running_mean: Optional[Tensor], running_var: Optional[Tensor], training: _bool,
        # momentum: _float, eps: _float, ,cudnn_enabled: _bool)
        ...
        # -----------貌似直接一个reshape函数就可以---------------#
        x1 = torch.sigmoid(self.b3(self.l3(x))[:, 0])
        x1 = x1.clamp(0, self.max_speed)
        x1 = x1.unsqueeze(1)
        x2 = torch.tanh(self.b3(self.l3(x))[:, 1])
        x2 = x2.unsqueeze(1)
