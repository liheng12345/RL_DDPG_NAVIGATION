...
class Agent:
    ...
    def __init__(self, stateSize, actionSize):
        ...
        self.savePath = '/home/lh/turtlebot3Model/'  # Model save path
    ...
    def trainModel(self, target=False):
        ...
        for i in range(self.batchSize):
            ...
            if target:
                nextTarget = self.targetModel.predict(
                    nextState.reshape(1, len(nextState)))
            else:
                nextTarget = self.onlineModel.predict(
                    nextState.reshape(1, len(nextState)))
            ...
            if done:
                xBatch = np.append(xBatch,
                                   np.array([nextState.copy()]),
                                   axis=0)
                yBatch = np.append(yBatch,
                                   np.array([[reward] * self.actionSize]),
                                   axis=0)

        self.onlineModel.fit(xBatch,
                             yBatch,
...
if __name__ == '__main__':
    ...
    # 从文件中加载模型
    # if agent.loadModel:
    #     agent.onlineModel.set_weights(
    #         load_model(agent.savePath + str(agent.loadEpisodeFrom) +
    #                    ".h5").get_weights())

    #     with open(agent.savePath + str(agent.loadEpisodeFrom) +
    #               '.json') as outfile:
    ...
    #         avg_max_q_val_text = "Avg Max Q Val:{:.2f}  | ".format(
    #             np.max(agent.qValue))
    ...
    #             print(
    #                 'Ep: {} | AvgMaxQVal: {:.2f} | CScore: {:.2f} | Mem: {} | Epsilon: {:.2f} | Time: {}:{}:{}'
    #                 .format(episode, avg_max_q, score, len(agent.memory),
    #                         agent.epsilon, h, m, s))
